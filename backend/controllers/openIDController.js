...
class openIDController {
  generateRedirectUri = async (req, next) => {
    try {
      ...
      params.append(
        'redirect_uri',
        `${process.env.API_DOMAIN}${process.env.API_URL}/auth/google/callback`
      )
  ...
  async handleCode(req, res, next) {
    try {
      ...
      if (error) {
        return res.redirect(
          302,
          `${process.env.CLIENT_DOMAIN}/auth/google?error=${error}`
        )
      }
      ...
      if (!response.ok) {
        ...
        return res.redirect(
          302,
          `${process.env.CLIENT_DOMAIN}/auth/google?error=${error.error}`
        )
      }
      ...
      return res.redirect(
        302,
        `${process.env.CLIENT_DOMAIN}/auth/google?code=${tempCode}`
      )
    } catch (e) {
      next(e)
    }
